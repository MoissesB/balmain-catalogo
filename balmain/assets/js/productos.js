(function () {
  ...
  const ui = {
    ...
    defaultProductMessage: {
      ...
      fr: "Bonjour, je souhaite recevoir des informations professionnelles sur le modèle {product} de Balmain Eyewear et connaître les disponibilités ainsi que les variantes proposées.",
    },
    normal: { es: "Normal", en: "Normal", fr: "Normal" },
    hidden: { es: "Oculto temporalmente", en: "Temporarily unavailable", fr: "Temporairement indisponible" },
    soldout: { es: "Agotado", en: "Unavailable", fr: "Indisponible" },
    lowstock: { es: "Pocas unidades", en: "Limited availability", fr: "Disponibilité limitée" },
    checkNextAvailability: { es: "Consultar proxima disponibilidad", en: "Request the next availability date", fr: "Demander la prochaine disponibilité" },
    ...
    modelsAvailable: { es: "modelos disponibles", en: "models available", fr: "modèles disponibles" },
    commercialCatalogue: { es: "catalogo comercial", en: "commercial catalogue", fr: "catalogue commercial" },
    materialsSellingCodes: { es: "Materiales y codigos de venta", en: "Materials and commercial codes", fr: "Matières et codes commerciaux" },
    buyerArguments: { es: "Argumentos utiles para compradores profesionales", en: "Commercial guidance for professional buyers", fr: "Arguments commerciaux pour les acheteurs professionnels" },
    ...
    materialsInquiry: {
      ...
      en: "Material details are available from Innova’s commercial team.",
      fr: "Les informations sur les matières sont disponibles auprès de l’équipe commerciale d’Innova.",
    },
    selectedVariant: { es: "Variante seleccionada", en: "Selected variant", fr: "Variante sélectionnée" },
    ...
    distributedByInnova: { es: "Distribuido por Innova", en: "Distributed by Innova", fr: "Distribué par Innova" },
    selectedCode: { es: "Codigo seleccionado", en: "Selected reference", fr: "Référence sélectionnée" },
    color: { es: "Color", en: "Colourway", fr: "Coloris" },
    ...
    professionalBuying: { es: "para compra profesional", en: "for professional selection", fr: "pour la sélection professionnelle" },
    ...
    technicalDetails: { es: "Detalles tecnicos", en: "Technical details", fr: "Détails techniques" },
    specificationsCatalogue: { es: "Especificaciones del catalogo", en: "Specifications from catalogue", fr: "Specifications du catalogue" },
    materials: { es: "Materiales", en: "Materials", fr: "Matières" },
    commercialTalkingPoints: { es: "Argumentos comerciales", en: "Commercial talking points", fr: "Arguments commerciaux" },
    requestAvailabilityInnova: { es: "Solicitar disponibilidad con Innova", en: "Request availability from Innova", fr: "Demander les disponibilités à Innova" },
    premiumAssets: { es: "Presentacion premium y estuches", en: "Premium presentation and cases", fr: "Présentation haut de gamme et étuis" },
    packagingCopy: {
      ...
      fr: "Le catalogue SS26 présente les solutions de conditionnement haut de gamme destinées aux présentations professionnelles et à la coordination des livraisons avec Innova.",
    },
    related: { es: "Relacionados", en: "Related models", fr: "Modèles associés" },
    sameCategory: { es: "Misma categoria", en: "Same category", fr: "Même catégorie" },
    previousProducts: { es: "Productos anteriores", en: "Previous products", fr: "Produits précédents" },
    nextProducts: { es: "Productos siguientes", en: "Next products", fr: "Produits suivants" },
  };
  ...
  async function getProducts() {
    if (!cache.products) {
      const response = await fetch(url("data/productos.json"), { cache: "no-store" });
  ...
  function variantImages(variant) {
    ...
    (Array.isArray(map.personas) ? map.personas : []).forEach((src, index) => {
      if (src && !seen.has(src)) {
        seen.add(src);
        images.push({
          src,
      ...
      <article class="product-card ${compact ? "compact" : ""} inventory-${status}" data-product-slug="${escapeHtml(product.slug)}">
        ...
        <div class="product-card-copy">
          ...
          <div class="card-actions">
            ...
            <button class="order-quick-add" type="button" data-order-add data-order-slug="${escapeHtml(product.slug)}" data-order-variant="0">Añadir al pedido</button>
          </div>
  ...
  async function renderProduct() {
    ...
    const visibleCatalogue = visibleProducts(products).filter((item) => item.slug !== product.slug);
    const relatedSlugs = new Set(product.relacionados || []);
    const related = [
      ...visibleCatalogue.filter((item) => relatedSlugs.has(item.slug)),
      ...visibleCatalogue.filter((item) => item.categoria === product.categoria),
      ...visibleCatalogue,
    ...
    target.innerHTML = `
      ...
      ${related.length ? `
        <section class="section related-products related-products-carousel" data-related-carousel>
          <div class="section-heading related-products-heading">
            <div>
              <p class="eyebrow">${t("related")}</p>
              <h2>${t("sameCategory")}</h2>
            </div>
            <div class="related-carousel-navigation">
              <span data-related-page>01 / ${String(Math.max(1, Math.ceil(related.length / 3))).padStart(2, "0")}</span>
              <button type="button" data-related-previous aria-label="${t("previousProducts")}" disabled>&larr;</button>
              <button type="button" data-related-next aria-label="${t("nextProducts")}">&rarr;</button>
            </div>
          </div>
    ...
    document.getElementById("addToBoutiqueOrder")?.addEventListener("click", (event) => {
      window.BalmainOrder?.addProduct(product, selectedVariantIndex, 1);
      const button = event.currentTarget;
      button.textContent = "Añadir más unidades";
      button.classList.add("button-boutique-added");
    });
    ...
    bindProductZoom();
    updateVariant(0);
    bindRelatedProductsCarousel();
  }

  function bindRelatedProductsCarousel() {
    const carousel = document.querySelector("[data-related-carousel]");
    const track = carousel?.querySelector("[data-related-track]");
    const previous = carousel?.querySelector("[data-related-previous]");
    const next = carousel?.querySelector("[data-related-next]");
    const pageLabel = carousel?.querySelector("[data-related-page]");
    if (!carousel || !track || !previous || !next) return;

    let page = 0;
    const productCount = track.children.length;
    const perPage = () => window.matchMedia("(max-width: 680px)").matches ? 1 : 3;
    const totalPages = () => Math.max(1, Math.ceil(productCount / perPage()));
    const update = (nextPage) => {
      page = Math.max(0, Math.min(totalPages() - 1, nextPage));
      track.style.transform = `translateX(calc(-${page * 100}% - ${page * 18}px))`;
      previous.disabled = page === 0;
      next.disabled = page === totalPages() - 1;
      if (pageLabel) {
        pageLabel.textContent = `${String(page + 1).padStart(2, "0")} / ${String(totalPages()).padStart(2, "0")}`;
      }
    };
