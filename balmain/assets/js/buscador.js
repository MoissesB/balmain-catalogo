(function () {
  const adminTrigger = "balmain";
  const adminKey = "balmain";
  ...
  function adminAccessModal(openPanel) {
    ...
    copy.textContent = "Ingresa la clave de la marca para abrir el portal independiente de Balmain.";
    error.textContent = "";
    input.value = "";
    modal.hidden = false;
    input.focus();
  ...
  function brandAdminPanel(allProducts) {
    let panel = document.querySelector("[data-brand-admin]");
    if (!panel) {
      panel = document.createElement("div");
      panel.className = "admin-access-overlay";
      panel.hidden = true;
      panel.dataset.brandAdmin = "";
      panel.innerHTML = `
        <section class="admin-access-card brand-admin-card" role="dialog" aria-modal="true" aria-label="Portal Balmain">
          <button class="icon-button admin-access-close" type="button" data-brand-admin-close aria-label="Cerrar">x</button>
          <p class="eyebrow">Portal independiente · Balmain</p>
          <h2>Gestión de marca</h2>
          <p>Administra publicaciones del blog y la visibilidad del catálogo Balmain sin mezclar la información con otras marcas.</p>
          <div class="brand-admin-actions">
            <button class="button button-dark" type="button" data-brand-admin-action="blog">Publicaciones del blog</button>
            <button class="button button-outline" type="button" data-brand-admin-action="inventory">Visibilidad de productos</button>
          </div>
        </section>
      `;
      document.body.appendChild(panel);
      panel.querySelector("[data-brand-admin-close]").addEventListener("click", () => {
        panel.hidden = true;
      });
      panel.addEventListener("click", (event) => {
        if (event.target === panel) panel.hidden = true;
      });
      panel.querySelector("[data-brand-admin-action='blog']").addEventListener("click", () => {
        panel.hidden = true;
        window.BalmainBlog?.openAdmin();
      });
      panel.querySelector("[data-brand-admin-action='inventory']").addEventListener("click", () => {
        panel.hidden = true;
        inventoryPanel(allProducts);
      });
    }
    panel.hidden = false;
  }
  ...
  async function initSearch() {
    ...
    input.addEventListener("keydown", (event) => {
      if (event.key === "Enter" && input.value.trim().toLowerCase() === adminTrigger) {
        event.preventDefault();
        adminAccessModal(() => brandAdminPanel(allProducts));
      }
    });
